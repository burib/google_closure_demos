...
# Changes:
# - added finer closure compiler arguments handling - ZsoltM
# - fixed deps.js paths which contained backslash seprated strings on Win
...
def GetDepsLine(dep, base_path):
  ...
  return 'goog.addDependency("%s", %s, %s);' % (
      '/'.join(os.path.normpath(
          GetRelpath(dep.filename, base_path)).split(os.sep)),
...
def PrintDeps(source_paths, deps, out):
  ...
  PrintLine('// This file was autogenerated by calcdeps_hcom.py, please do not edit', out)
...
def Compile(compiler_jar_path, source_paths, outFile, flags=None):
...
def var_arg_list(option, opt_str, value, parser):
    """Processes variable argument command line options, intended to 
    be used as a callback for optparse module"""
    ...
    del parser.rargs[:len(value) + shift]
...
def main():
  ...
  parser.add_option('--compiler-args',
                    action='callback',
                    callback=var_arg_list,
                    dest='compiler_flags',
                    help='Any arguments following this option are to be passed to Closure compiler. '
                    'Two hypens (--) can be used to mark the end of argument list.')
  ...
  if options.output_mode != 'compiled':
  ...
  if output_mode == 'script':
    ...
    Compile(options.compiler_jar, deps, options.output_file, options.compiler_flags)
